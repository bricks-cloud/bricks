...
const rgbToHex = (color: RGB): string => {
    return "#" + componentToHex(Math.round(color.r * 255)) + componentToHex(Math.round(color.g * 255)) + componentToHex(Math.round(color.b * 255));
};

const getGradientColors = (gradientStops: readonly ColorStop[]): [string, number][] => {
    const results: [string, number][] = [];
    for (const gradientStop of gradientStops) {
        results.push([rgbToHex(gradientStop.color), Math.trunc(gradientStop.position * 10) / 10]);
    }
